...
class Node:
    ...
    def GetInvolve(self, invReqMsg: InvolveRequestMsg):
        ...
        if invReqMsg.Scheme == 0:
            ...
            if self.NodeID == self.View.Primary:
                with open('test0.csv', 'a+', encoding='utf-8', newline="") as file_obj:
                    writer = csv.writer(file_obj)
                    writer.writerow(
                        [self.NodeID, "START", round(time.time()*1000)])
            ...
            if self.NodeID == invReqMsg.NodeID:
                with open('testDRC.csv', 'a+', encoding='utf-8', newline="") as file_obj:
                    writer = csv.writer(file_obj)
                    writer.writerow(
                        [self.NodeID, "START", round(time.time()*1000)])
                LogStage("InvolveForward", False)
    ...
    def GetInvolvePrepare(self, prepareMsg: VoteMsg):
        if self.CurrentState is not None:
            ...
            if self.NodeID == self.CurrentState.MsgLogs.ReqMsg.NodeID:
                ...
                if self.CurrentState.InvolvePrepare(prepareMsg, self.CurrentState.MsgLogs.InvReqMsgs[0], self.CurrentState.MsgLogs.PrepareMsgs):
                ...
                if self.CurrentState.InvolvePrepare(prepareMsg, self.LastConsensusMsgs.ReqMsg, self.LastConsensusMsgs.PrepareMsgs):
    ...
    def GetInvolveCommit(self, commitMsg: VoteMsg):
        if self.CurrentState is not None:
            if self.NodeID == self.CurrentState.MsgLogs.ReqMsg.NodeID:
                ...
                if self.CurrentState.InvolveCommit(commitMsg, self.CurrentState.MsgLogs.InvReqMsgs[0], self.CurrentState.MsgLogs.CommitMsgs):
                    invReq = self.CurrentState.MsgLogs.ReqMsg
                    self.CurrentState.MsgLogs.ReqMsg = self.CurrentState.MsgLogs.InvReqMsgs[0]
                    # (commitMsg, preResult) = self.CurrentState.Prepare(self.NodeID, prepareMsg)
                    (replyMsg, commitedMsg, globalForwardMsg,
                ...
                if self.CurrentState.InvolveCommit(commitMsg, self.LastConsensusMsgs.ReqMsg, self.LastConsensusMsgs.CommitMsgs):
    ...
    def GlobalShare(self, globalForwardMsg: GlobalForwardMsg, sourceStage):
        if sourceStage == "InvolveForward" and self.CurrentState is not None:
            ...
            with open('test0.csv', 'a+', encoding='utf-8', newline="") as file_obj:
                writer = csv.writer(file_obj)
                writer.writerow([self.NodeID, "END", round(time.time()*1000)])
            # pass
        else:
