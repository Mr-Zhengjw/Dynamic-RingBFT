...
class State:
    ...
    def PrePrepare(self, nodeID, prePrepareMsg: PrePrepareMsg):
        ...
        preMsg = VoteMsg(self.ViewID, prePrepareMsg.SequenceID,
                         prePrepareMsg.Digest, nodeID, MsgType.PREPAREMSG, True)
    ...
    def Prepare(self, nodeID, prepareMsg: VoteMsg):
        ...
        if Prepared(self):
            ...
            if voteCount <= (len(self.MsgLogs.PrepareMsgs) - voteCount):
                ...
                comMsg = VoteMsg(self.ViewID, prepareMsg.SequenceID,
                                 prepareMsg.Digest, nodeID, MsgType.COMMITMSG, True)
    ...
    def InvolveVerifyReq(self, nodeID, request: RequestMsg):
        ...
        preMsg = VoteMsg(self.ViewID, request.SequenceID, dig,
                         nodeID, MsgType.INVOLVEPREPAREMSG, True)
        self.MsgLogs.PrepareMsgs[nodeID] = preMsg
        # 进行投票
        return preMsg

    # 申请阶段的Prepare阶段
    def InvolvePrepare(self, prepareMsg: VoteMsg, reqMsg: RequestMsg, prepareMsgs: dict):
        # print("InvolvePrepare",flush=True)
    ...
    # 申请阶段的Commit阶段
    def InvolveCommit(self, commitMsg: VoteMsg, reqMsg: RequestMsg, commitMsgs: dict):
...
def VerifyMsg(state: State, viewID: int, sequenceID: int, digGot: str):
    ...
    if state.LastSequenceID != -1 and state.LastSequenceID >= sequenceID:
        print(
            "state.LastSequenceID != -1 and state.LastSequenceID >= sequenceID", flush=True)
        return False
...
def Prepared(state: State):
    ...
    if len(state.MsgLogs.PrepareMsgs) <= 2*f:
        print("len(state.MsgLogs.PrepareMsgs) {}".format(
            len(state.MsgLogs.PrepareMsgs)), flush=True)
        return False
    return True
